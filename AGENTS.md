# AGENTS.md — Apache DolphinScheduler
...
**This file is an index.** Each module has its own `AGENTS.md` with the details — do not duplicate module contents here.
...
|---------|--------|------------|---------------|
| **API** | [`dolphinscheduler-api`](dolphinscheduler-api/AGENTS.md) | `org.apache.dolphinscheduler.api.ApiApplicationServer` | `12345` (HTTP / UI + REST) |
| **Master** | [`dolphinscheduler-master`](dolphinscheduler-master/AGENTS.md) | `org.apache.dolphinscheduler.server.master.MasterServer` | `5679` (RPC) |
| **Worker** | [`dolphinscheduler-worker`](dolphinscheduler-worker/AGENTS.md) | `org.apache.dolphinscheduler.server.worker.WorkerServer` | `1235` (RPC) |
| **Alert** | [`dolphinscheduler-alert`](dolphinscheduler-alert/AGENTS.md) (→ `-alert-server`) | `org.apache.dolphinscheduler.alert.AlertServer` | `50053` (HTTP), `50052` (RPC) |
| Standalone (dev only) | [`dolphinscheduler-standalone-server`](dolphinscheduler-standalone-server/AGENTS.md) | `org.apache.dolphinscheduler.StandaloneServer` | `12345` + `50052` (API + alert; master/worker use in-JVM calls) |
...
Click into a module's `AGENTS.md` for details. Each description is one line here on purpose.

### Core execution

- [`dolphinscheduler-master`](dolphinscheduler-master/AGENTS.md) — workflow orchestration engine; consumes `Command`s, runs the DAG state machine, dispatches to workers.
- [`dolphinscheduler-worker`](dolphinscheduler-worker/AGENTS.md) — runs physical tasks dispatched from master; hosts task plugins.
- [`dolphinscheduler-task-executor`](dolphinscheduler-task-executor/AGENTS.md) — reusable task-lifecycle framework embedded by the worker.
- [`dolphinscheduler-alert`](dolphinscheduler-alert/AGENTS.md) — alert server + channel plugins (email, Feishu, DingTalk, …).

### API layer

- [`dolphinscheduler-api`](dolphinscheduler-api/AGENTS.md) — REST API server (entry point for UI, Python SDK, external clients).
- [`dolphinscheduler-api-test`](dolphinscheduler-api-test/AGENTS.md) — integration tests against the REST API (Docker Compose + Testcontainers).
- [`dolphinscheduler-authentication`](dolphinscheduler-authentication/AGENTS.md) — Actuator-endpoint auth + AWS credential helpers (NOT the main login path).

### Shared libraries

- [`dolphinscheduler-common`](dolphinscheduler-common/AGENTS.md) — foundation utilities (everything depends on this).
- [`dolphinscheduler-dao`](dolphinscheduler-dao/AGENTS.md) — MyBatis DAO layer + SQL migration scripts.
- [`dolphinscheduler-service`](dolphinscheduler-service/AGENTS.md) — business logic between DAO and the servers.
- [`dolphinscheduler-spi`](dolphinscheduler-spi/AGENTS.md) — Service-Provider Interface root (every plugin depends on this).
- [`dolphinscheduler-extract`](dolphinscheduler-extract/AGENTS.md) — RPC interface contracts between servers.
- [`dolphinscheduler-eventbus`](dolphinscheduler-eventbus/AGENTS.md) — in-process event-bus abstractions.
- [`dolphinscheduler-registry`](dolphinscheduler-registry/AGENTS.md) — pluggable registry (Zookeeper / Etcd / JDBC).
- [`dolphinscheduler-meter`](dolphinscheduler-meter/AGENTS.md) — metrics (Prometheus) + server load-protection primitives.

### Plugin families

- [`dolphinscheduler-task-plugin`](dolphinscheduler-task-plugin/AGENTS.md) — task-type plugins (shell, SQL, Spark, Flink, K8s, EMR, …). 33 concrete plugins.
- [`dolphinscheduler-datasource-plugin`](dolphinscheduler-datasource-plugin/AGENTS.md) — user-facing datasource plugins (MySQL, Hive, Trino, Snowflake, …). 28 concrete plugins.
- [`dolphinscheduler-storage-plugin`](dolphinscheduler-storage-plugin/AGENTS.md) — resource storage (S3, HDFS, OSS, GCS, ABS, OBS, COS).
- [`dolphinscheduler-scheduler-plugin`](dolphinscheduler-scheduler-plugin/AGENTS.md) — cron scheduler (Quartz today).
- [`dolphinscheduler-dao-plugin`](dolphinscheduler-dao-plugin/AGENTS.md) — metadata-DB dialect support (MySQL / PostgreSQL / H2).

### Build, ops, tools

- [`dolphinscheduler-bom`](dolphinscheduler-bom/AGENTS.md) — Maven BOM; central dependency version pinning.
- [`dolphinscheduler-dist`](dolphinscheduler-dist/AGENTS.md) — assembles the release tarball + Docker images.
- [`dolphinscheduler-standalone-server`](dolphinscheduler-standalone-server/AGENTS.md) — all-in-one JVM with H2 (dev / smoke tests).
- [`dolphinscheduler-tools`](dolphinscheduler-tools/AGENTS.md) — CLIs for schema upgrade + resource / lineage migration.
- [`dolphinscheduler-microbench`](dolphinscheduler-microbench/AGENTS.md) — JMH micro-benchmarks.
- [`dolphinscheduler-yarn-aop`](dolphinscheduler-yarn-aop/AGENTS.md) — AspectJ weaver capturing YARN ApplicationIds.

### Frontend & E2E

- [`dolphinscheduler-ui`](dolphinscheduler-ui/AGENTS.md) — Vue 3 frontend.
- [`dolphinscheduler-e2e`](dolphinscheduler-e2e/AGENTS.md) — Selenium browser tests.
